...
# Import the extract_pdf_summary function and PDFSummarySchema from table_generate_agent.py
from table_generate_agent import extract_pdf_summary, PDFSummarySchema
...
def save_translated_word(md_content, images_output_folder, word_output_path, logo_output_folder, logo_fig_indices, pdf_summary: PDFSummarySchema):
    """
    Save a translated version of the document to a Word file, including a title and summary section.
    ...
    Args:
        ...
        logo_fig_indices (set): Set of figure indices identified as logos.
        pdf_summary (PDFSummarySchema): Extracted title and summary of the document.
    """
    # Clean up the markdown content
    md_content = clean_md_content(md_content)
    ...
    # Insert the PDF title and summary at the beginning with labels
    if pdf_summary:
        # Add title with label
        if pdf_summary.title:
            title_para = translated_doc.add_paragraph()
            title_para.add_run("Title: ").bold = True
            title_para.add_run(pdf_summary.title)
        
        # Add summary with label
        if pdf_summary.summary:
            summary_para = translated_doc.add_paragraph()
            summary_para.add_run("Summary: ").bold = True
            summary_para.add_run(pdf_summary.summary)
            
        # Add spacing after the summary
